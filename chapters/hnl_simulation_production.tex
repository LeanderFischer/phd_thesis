...
\begin{equation}
    ...
    \labeq{weight_full}
\end{equation}
If this additional weighting factor is multiplied to a generation weight (like in \refeq{neutrino_generation_weight}), the livetime, and the oscillated primary neutrino flux, it results in the number of expected events in the detector for this particular MC event for a chosen mixing (and mass).

\todo{add table with number of gen level files? mention the event number is smaller becasue of kinemtic condition?}
