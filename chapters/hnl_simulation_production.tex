...
\todo{JVS:
    consider remaking figures at the native size so type size is consistent. Also, consider making the legend box semi-transparent so the lines do not obscure the text and markers. (ORANGE)
}
...
\end{margintable}
\todo{Calculate max BRs or remove.. (RED)}
...
\todo{What is the message of these plots? Make it clear in the text and also in the captions. Potentially cut down to one or two and leave the rest or move them to the appendix. (RED)}
