...
\begin{table}
    \small
        ...
        \textbf{Variable} & \textbf{Threshold} & \textbf{\si{\percent} removed} \\ 
    
        \hline\hline
    
        Number of hit DOMs & $\geq 7$ & \\
        Radial distance & < \SI{200}{\meter} & \\
        Vertical position & \SI{-495}{\meter} < z < \SI{-225}{\meter} & \\
        Energy & \SI{5}{\giga\electronvolt} < E < \SI{100}{\giga\electronvolt} & \\
    
        Cosine of zenith angle & < 0.04 & \\
        Number of direct hits & > 2.5 & \\
        Number of hits in top layers & < 0.5 & \\
        Number of hits in outer layer & < 7.5 & \\
        Muon classifier score & $\geq 0.8$ & \\
...
- the uncertainties in the form factors are due to uncertainties in the \textit{axial mass} $M_\rm{A}$ which enters the form factor as in 
...
- the axial mass can be determined experimentally and to include uncertainties on the values of $M_\rm{A}^\rm{CCQE}$ and $M_\rm{A}^\rm{CCRES}$, the cross-sections are computed with \textsc{GENIE} where the form factors are calculated varying the axial mass by $\pm 20\% (1\sigma)$/$\pm 40\% (1\sigma)$ around the nominal value
- this is an approximation of the recommended uncertainties by the GENIE collaboration, which are $-15\%$, $+25\%$ for $M_\rm{A}^\rm{CCQE}$ and $\pm 20\%$ for $M_\rm{A}^\rm{CCRES}$ \cite{genie}
...
\todo{add DIS systematic effect on final level histograms}
