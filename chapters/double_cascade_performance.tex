...
\begin{itemize}
    ...
    \item Criteria: Taupede has to be run succesfull (good), Selection up to L7 has to be passed (L7), both cascades have to be reconstructed above certain energy (reco E1,E2 above 3 GeV)
    \item 2d hist of energies compares reconstructed (electromagnetic) energy to the true energy, so a bias is expected becasue some part goes into EM neutral particles (that don't produce ligtht)
    \item both cascade energies are looking good above ~ 10 GeV, although the second cascade energy is much wider spread
    \item the bin count shows that the second cascade energies are much lower as compared to the first
    \item 
\end{itemize}
...
\begin{figure*}[h]
    ...
    \caption[]{}
    \labfig{selected_routine_length_results_3_gev}
\end{figure*}
\todo{fix caption of this figure}
...
\begin{figure*}[h]
    ...
    \caption[]{}
    \labfig{selected_routine_length_results_10_gev}
\end{figure*}
\todo{fix caption of this figure}
