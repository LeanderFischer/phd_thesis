...
\paragraph{Fit Routine:}

It was observed that the likelihood space can be very dependent on the current parameter set $\Theta_i$\todo{Add a section showing some likelihood scans with several parameters and the effect on the decay length LLH.} and especially the likelihood distribution w.r.t. the decay length. Seeding the decay length with a value much different from the true value can lead to a resulting best fit in a local minimum. To get a better estimate of the decay length a fit routine was tested where some of the parameters were fixed in the first iteration and then the resulting best fit point was used to fit all 9 parameters in a second iteration. \cref{fig:reconstruction_optimization_fit_routine} shows the median, absolute, fractional decay length resolution for two tested fit routines. It can be seen how a fit split into two consecutive steps, where the first step fits only both cascade energies and the decay length and the second step fits the full 9 parameters, performs as compared to just the full 9 parameter fit. It can be seen that the iterative fit approach seems to result in a better resolution. The initial seed for both routines is the same.\todo{Put down some information about the used seed parameters.}

% \begin{figure}[h]
%     \centering
%     \includegraphics[width=0.8\textwidth]{figures/reconstruction/optimization_190605/fit_routine/median_decay_length_resolution_Good + L7 + reco E1,E2 above 3_fix_y_n_events.png}
%     \caption{Decay length resolution versus true decay length comparing a full 9 paramters fit to an iterative approach where first the energies and the decay lenght are fitted, while fixing the other 7 parameters and then the full fit is performed.
%     This was done with older signal simulation set (190605) and the resolution is unweighted.}
%     \label{fig:reconstruction_optimization_fit_routine}
% \end{figure}


\paragraph{Decay Length Seeds:}

Following the same line of argument as with the iterative fit approach, it was tested how multiple fits seeded with different decay length values might improve the resolution, comparing to a single decay length seed fit. In \cref{fig:reconstruction_optimization_decay_length_seeding} it can be seen that a fit with multiple decay length seed achieves slightly better results.

% \begin{figure}[h]
%     \centering
%     \includegraphics[width=0.8\textwidth]{figures/reconstruction/optimization_190605/length_seeds/median_decay_length_resolution_Good + L7 + reco E1,E2 above 3_fix_y.png}
%     \caption{Decay length resolution versus true decay length comparing the same fit routine seeded with just the seed decay length (Single length seed) and seeded with a decay length of 5\,m, 25\,m, 50\,m, 100\,m, and 200\,m (Multiple length seeds).
%     This was done with older signal simulation set (190605) and the resolution is unweighted.}
%     \label{fig:reconstruction_optimization_decay_length_seeding}
% \end{figure}
...
% \begin{figure}[h]
%     \centering
%     \includegraphics[width=0.8\textwidth]{figures/reconstruction/optimization_190605/minimizers/median_decay_length_resolution_Good + L7 + reco E1,E2 above 3_n_events.png}
...
%     \label{fig:reconstruction_optimization_minimizers}
% \end{figure}
...
The runtimes for these fit variations was also tracked alongside with the number of succesfull fits.\todo{Add a table with fit runtimes and success rates?}
